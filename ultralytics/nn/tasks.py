...
def attribute_assignment(model_pt, device=None, inplace=True, fuse=False):
    """Load model weights and set attributes."""
    # Load model checkpoint and path
    ckpt, model_pt = load_download_model(model_pt)
    model = (ckpt.get("ema") or ckpt["model"]).to(device).float()  # Load FP32 model

    # Attach attributes to the model
    model.model_name = model_pt
    model.task = guess_model_task(model)
    model.args = {k: v for k, v in {**DEFAULT_CFG_DICT, **ckpt.get("train_args", {})}.items() if k in DEFAULT_CFG_KEYS}

    # Set default stride if not present
    model.stride = getattr(model, "stride", torch.tensor([32.0]))

    # Apply fusion and set evaluation mode
    ...
    for m in model.modules():
        if hasattr(m, "inplace"):
            ...
            m.recompute_scale_factor = None  # For torch 1.11.0 compatibility
