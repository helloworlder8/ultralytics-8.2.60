...
class Predictor(BasePredictor):
    ...
    def preprocess(self, image):
        ...
        Args:
            image (torch.Tensor | List[np.ndarray]): BCHW tensor format or list of HWC numpy arrays.

        Returns:
            (torch.Tensor): The preprocessed image tensor.
        """
        if self.image is not None:
            return self.image #原本就有图像的话直接返回
        not_tensor = not isinstance(image, torch.Tensor)
        if not_tensor:
            image = np.stack(self.pre_transform(image)) #->(1, 1024, 1024, 3)
        ...
        return image #torch.Size([1, 3, 1024, 1024])
    ...
    def pre_transform(self, image):
        ...
        Args:
            image (List[np.ndarray]): List containing images in HWC numpy array format.

        Returns:
            (List[np.ndarray]): List of transformed images.
        """
        assert len(image) == 1, "SAM model does not currently support batched inference"
        letterbox = LetterBox(self.args.imgsz, auto=False, center=False)
        return [letterbox(image=x) for x in image]

    def inference(self, image, bboxes=None, points=None, labels=None, masks=None, multimask_output=False, *args, **kwargs):
        """
        ...
        Args:
            image (torch.Tensor): The preprocessed input image in tensor format, with shape (N, C, H, W).
        ...
        if all(i is None for i in [bboxes, points, masks]):
            return self.generate(image, *args, **kwargs)

        return self.prompt_inference(image, bboxes, points, labels, masks, multimask_output)

    def prompt_inference(self, image, bboxes=None, points=None, labels=None, masks=None, multimask_output=False):
        """
        ...
        Returns:
            (tuple): Contains the following three elements.
                ...
                - np.ndarray: Low-resolution logits of shape CxHxW for subsequent inference, where H=W=256.
        """
        features = self.model.image_encoder(image) if self.features is None else self.features #->torch.Size([1, 256, 64, 64])

        src_shape, dst_shape = self.batch[1][0].shape[:2], image.shape[2:]
    ...
    def generate(
        ...
        Args:
            image (torch.Tensor): Input tensor representing the preprocessed image with dimensions (N, C, H, W).
        ...
        for crop_region, layer_idx in zip(crop_regions, layer_idxs):
            ...
            w, h = x2 - x1, y2 - y1
            area = torch.tensor(w * h, device=image.device)
            points_scale = np.array([[w, h]])  # w, h
            # Crop image and interpolate to input size
            crop_im = F.interpolate(image[..., y1:y2, x1:x2], (ih, iw), mode="bilinear", align_corners=False)
            # (num_points, 2)
    ...
    def setup_model(self, model, verbose=True):
        ...
        if model is None:
            model = build_sam(self.args.model_name)
        model.eval()
    ...
    def postprocess(self, preds, image, orig_imgs):
        """
        ...
        Args:
            ...
            image (torch.Tensor): The processed input image tensor.
        ...
        for i, masks in enumerate([pd_masks]):
            ...
            if pd_bboxes is not None:
                pd_bboxes = ops.scale_boxes(image.shape[2:], pd_bboxes.float(), orig_img.shape, padding=False)
    ...
    def setup_source(self, source):
        ...
        if source is not None:
            super().setup_source(source) #'demo/bedroom.mp4'
...
class SAM2VideoPredictor(SAM2Predictor):
    ...
    def postprocess(self, preds, image, orig_imgs):
        ...
        """
        results = super().postprocess(preds, image, orig_imgs)
