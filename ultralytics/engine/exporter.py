...
class Exporter:
    ...
    def export_torchscript(self, prefix=colorstr("TorchScript:")):
        ...
        ts = torch.jit.trace(self.model, self.image, strict=False)
    ...
    def export_onnx(self, prefix=colorstr("ONNX:")):
        ...
        torch.onnx.export(
            ...
            self.image.cpu() if dynamic else self.image,
            f,
            verbose=False,
    ...
    def export_openvino(self, prefix=colorstr("OpenVINO:")):
        ...
        ov_model = ov.convert_model(
            self.model,
            input=None if self.args.dynamic else [self.image.shape],
            example_input=self.image,
        )
    ...
    def export_paddle(self, prefix=colorstr("PaddlePaddle:")):
        ...
        pytorch2paddle(module=self.model, save_dir=f, jit_type="trace", input_examples=[self.image])  # export
    ...
    def export_coreml(self, prefix=colorstr("CoreML:")):
        ...
        if self.model.task == "classify":
            ...
            model = IOSDetectModel(self.model, self.image) if self.args.nms else self.model
        else:
            if self.args.nms:
        ...
        ts = torch.jit.trace(model.eval(), self.image, strict=False)  # TorchScript model
        ct_model = ct.convert(
            ts,
            inputs=[ct.ImageType("image", shape=self.image.shape, scale=scale, bias=bias)],
    ...
    def export_engine(self, prefix=colorstr("TensorRT:")):
        ...
        assert self.image.device.type != "cpu", "export running on CPU but must be on GPU, i.e. use 'device=0'"
    ...
    def _pipeline_coreml(self, model, weights_dir=None, prefix=colorstr("CoreML Pipeline:")):
        ...
        _, _, h, w = list(self.image.shape)  # BCHW
