...
class BaseProject(nn.Module):
    ...
    def _load(self, model_pt: str, task=None) -> None:
        ...
        if Path(model_pt).suffix == ".pt":
            ...
            self.overrides = self.model.args = self._reset_ckpt_args(self.model.args) #任务 数据配置 图像尺寸 单一类 训练模式下不需要这么多参数
        else:
    ...
    def predict(
        ...
        **kwargs,
        ...
        custom = {"conf": 0.001, "batch": 1, "save": False, "mode": "predict"}  # 人任务模型数据，图单
    ...
    def _combine_args(self, kwargs):
        ...
        if self.args.get("resume"): #有这个标记的话
            self.args["resume_pt"] = self.model_name #从前面传来的模型开始训练
    ...
    def _reset_ckpt_args(args: dict) -> dict:
        ...
        include = {"imgsz", "single_cls", "data"}  # only remember these arguments when loading a PyTorch model
