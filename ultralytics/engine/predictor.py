...
class BasePredictor:
    ...
    def setup_source(self, source): #'demo/bedroom.mp4'
        """Sets up source and inference mode."""
        self.imgsz = check_imgsz(self.args.imgsz, stride=self.model.stride, min_dim=2)  # [1024, 1024]
        self.transforms = ( #None
            getattr(
                self.model.model,
                "transforms",
    ...
    def stream_inference(self, source=None, model_name=None, *args, **kwargs):
        ...
        with self._lock:  # for thread-safe inference
            ...
            for self.batch in self.dataset:
                ...
                # Preprocess
                with profilers[0]:
                    preprocessed_image = self.preprocess(ori_img) #填充 归一化 数组变成张量
                ...
                with profilers[1]:
                    ...
                    if 'ch' in kwargs: #点 标签 批
                        del kwargs['ch']
                        
                    preds = self.inference(preprocessed_image, *args, **kwargs) #torch.Size([1, 3, 1024, 1024]) 点 标签 批
                    if self.args.embed:
                ...
                # Postprocess
                with profilers[2]:
                    self.results = self.postprocess(preds, preprocessed_image, ori_img) #掩膜 得分 处理后的图像 原始图像
                ...
                for i in range(n):
                    ...
                    if self.args.verbose or self.args.save or self.args.save_txt or self.args.show:
                        s[i] += self.write_results(i, Path(paths[i]), preprocessed_image, s) #检测结果信息
        ...
        if self.args.verbose and self.seen:
            ...
            LOGGER.info(
                ...
                f"{(min(self.args.batch, self.seen), 3, *preprocessed_image.shape[2:])}" % t
            )
